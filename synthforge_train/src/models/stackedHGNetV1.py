...
from .unet_la import UNet
from .coord_conv import CoordConvTh
from .decoder_default import get_decoder
...
class Activation(nn.Module):
    def __init__(self, kind: str = "relu", channel=None):
        ...
        self.norm_fn = {
            ...
            "bn_noaffine": nn.BatchNorm2d(
                channel, affine=False, track_running_stats=True
            ),
            "none": None,
        }[norm_str]
...
class E2HTransform(nn.Module):
    def __init__(self, edge_info, num_points, num_edges):
        ...
        bias = (
            (e2h_matrix @ torch.ones(e2h_matrix.size(1)).to(e2h_matrix)) < 0.5
        ).to(e2h_matrix)
        # pn x 1.
...
class StackedHGNetV1(nn.Module):
    def __init__(
        ...
        self.use_unet_pre = use_unet_pre
        ...
        if use_unet_pre:
            self.pre = UNet(3, 10 + 1)  # inc bg class
        else:
        ...
        if self.cfg.use_AAM:
            ...
            self.e2h_transform = E2HTransform(
                edge_info, self.num_points, self.num_edges
            )
            self.out_edgemaps = nn.ModuleList(
                [
                    ConvBlock(
                        in_channel, self.num_edges, 1, relu=False, bn=False
                    )
                    for _ in range(nstack)
                ]
            )
            ...
            self.merge_edgemaps = nn.ModuleList(
                [
                    ConvBlock(
                        self.num_edges, in_channel, 1, relu=False, bn=False
                    )
    ...
    def forward(self, x):
        if self.use_unet_pre:
            seg, outs = self.pre(x, return_intermidiates=True)
            x = outs["x3"]
        else:
            x = self.pre(x)
        ...
        if self.use_unet_pre:
            return y, fusionmaps, landmarks, seg, outs
...
class StackedHGNetV2(nn.Module):
    def __init__(
        ...
        if self.add_coord:
            ...
            scale = int(np.log2(self.cfg.width // self.cfg.out_width))
            if scale > 0:
                convBlock = ConvBlock(
                    x_in_channel, 64, 7, 2, bn=True, relu=True
                )
                scale -= 1
            else:
                convBlock = ConvBlock(x_in_channel, 64, bn=True, relu=True)
        ...
        if not self.skip_pre:
            ...
            for i in range(scale):
                layers += [Block(in_feats, out_feats), pool]
                in_feats = out_feats
            layers += [Block(in_feats, out_feats), Block(out_feats, in_channel)]
            self.pre = nn.Sequential(*layers)

            # self.pre = nn.Sequential(
            #     convBlock, Block(64, 128), pool, Block(128, 128), Block(128, in_channel)
            # )
        ...
        self.hgs = nn.ModuleList(
            [
                Hourglass(
                    ...
                    add_coord=self.add_coord,
                    first_one=(_ == 0),
                    x_dim=self.cfg.out_width,
                    y_dim=self.cfg.out_height,
                )
                for _ in range(nstack)
            ]
        )
    ...
    def forward(self, x, kp_res=None, return_pre_out=False):
        if return_pre_out:
            assert self.skip_pre is False, "Pre layer is skipped"
        if not self.skip_pre:
            pre_x = self.pre(x)
            x = pre_x.clone()
        ...
        for i in range(self.nstack):
            ...
            fusionmaps.append(fusion_heatmaps)
        if return_pre_out:
            return (y, fusionmaps, pre_x)
        return y, fusionmaps

    @property
    def last_layers(self):
        return self.hgs, self.features, self.out_heatmaps

    def set_last_layers(self, hgs, features, out_heatmaps):
