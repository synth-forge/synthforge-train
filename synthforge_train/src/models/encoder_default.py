...
def get_encoder(
    image_height, image_width, scale=0.25, sigma=1.5, encoder_type="default"
):
...
class encoder_default:
    ...
    def _circle(self, img, pt, sigma=1.0, label_type="Gaussian"):
        ...
        g_x = list((max(0, -ul[0]), min(br[0], img.shape[1]) - ul[0]))
        g_y = list((max(0, -ul[1]), min(br[1], img.shape[0]) - ul[1]))
        # Image range
        img_x = list((max(0, ul[0]), min(br[0], img.shape[1])))
        img_y = list((max(0, ul[1]), min(br[1], img.shape[0])))
