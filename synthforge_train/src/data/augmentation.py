    ...
    def process(
        ...
        if self.is_train and random.random() < self.aug_prob:
            (
                ...
                flipped,
                *_seg,
            ) = self._flip.process(img, lmk, lmk_5pts, center_w, center_h, seg)
            if seg is not None:
                seg = _seg[0]
            ...
            if seg is not None:
                aug_seg, _ = self._transform.process(seg, lmk, matrix)
            ...
            if seg is not None:
                aug_seg, _ = self._transform.process(seg, lmk, matrix)
        rets = [aug_img, aug_lmk, matrix, flipped]

        if seg is not None:
            # aug_depth = self._transform._transformPerspective(aug_depth, matrix)
            rets.append(aug_seg)
        if depth is not None:
            aug_depth = cv2.resize(
                depth.copy(), img.shape[:-1], interpolation=cv2.INTER_LINEAR
            )
            aug_depth_mask = np.ones_like(aug_depth)
            if flipped:
                aug_depth = np.fliplr(aug_depth).copy()
                aug_depth_mask = np.fliplr(aug_depth_mask).copy()
            aug_depth = self._transform._transformPerspective(aug_depth, matrix)
            aug_depth_mask = self._transform._transformPerspective(
                aug_depth_mask, matrix
            )
            rets.append(np.stack([aug_depth, aug_depth_mask], axis=0))
        return rets
...
class GetCropMatrix:
    ...
    def _compose_rotate_and_scale(
        self, angle, scale, shift_xy, from_center, to_center
    ):
        cosv = math.cos(angle)
        sinv = math.sin(angle)
        ...
        rot_scale_m = np.array(
            [[a0, a1, a2], [b0, b1, b2], [0.0, 0.0, 1.0]], np.float32
        )
        return rot_scale_m
...
class GetAlignMatrix:
    def __init__(self, image_size, target_face_scale, std_lmk_5pts):
        ...
        """
        self.std_lmk_5pts = (
            (std_lmk_5pts * target_face_scale + 1)
            * np.array([image_size, image_size], np.float32)
            / 2.0
        )
...
class GetRandomGeometryMatrix:
    ...
    def process(self):
        ...
        if (
            self.shift_config[:2] != (0.0, 0.0)
            or self.rot_config[:2] != (0.0, 0.0)
            or self.scale_config[:2] != (1.0, 0.0)
        ):
            shift_xy = self._random(self.shift_config, size=[2]) * min(
                to_h, to_w
            )
...
class RandomTexture:
    ...
    def add_nose_fusion(self, image):
        h, w, c = image.shape
        nose = np.array(
            bytearray(os.urandom(h * w * c)), dtype=image.dtype
        ).reshape(h, w, c)
...
class Flip:
    ...
    def process(
        self, image, landmarks, landmarks_5pts, center_w, center_h, seg=None
    ):
        ...
        if landmarks_5pts is not None:
            ...
            landmarks_5pts_flip[:, 0] = (
                w + flip_offset - landmarks_5pts_flip[:, 0]
            )
        else:
